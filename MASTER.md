...
| M20 | **Kanban Surface Manager — Hermes-native execution surface over collab-mem** — dedicated Hermes Agent is KSM; native Kanban is the board/dispatcher; collab-mem remains project truth. Card: `projects/kanban-surface.md` | Codex | **active** | Live browser source/receipt view and selected Intake → Build dispatch verified; profile handoffs fixed in PRs #4–#5; native concurrency locked to one and auto-decompose disabled. Next = two-boot service acceptance, retire superseded KRP, then one Tony-watched UI drag. |
