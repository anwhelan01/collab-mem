...
| M20 | **Kanban Surface Manager — Hermes-native execution surface over collab-mem** — dedicated Hermes Agent is KSM; native Kanban is the board/dispatcher; collab-mem remains project truth. Card: `projects/kanban-surface.md` | Codex | **active** | 28 source-linked native cards projected on `alwyzon-1`; UI/gateway loopback live; WIP 1; zero model runs during import. Next = one selected native dispatch, browser/tunnel verification, two-boot acceptance, then retire the superseded KRP daemon. |
