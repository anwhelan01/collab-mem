...
| M18 | **Ask Ebbi 3 — the Planning Oracle** — Hermes-native Ask Ebbi product, Library MCP, 91k appeals. Card: `projects/ask-ebbi-3.md` | Tony / Ask Ebbi Codex | **active** | Healthy on the M4 at :8051 and public `askebbi.com` (Cloudflare tunnel); branch `ui-console-alt` latest `4c35c3f`/`2573361` (2026-07-20). Next = land the in-progress responsiveness fix (overflow + iPad portrait), then reconcile the canonical branch and deploy an isolated Compose project to `alwyz-01-dock`. |
| M19 | **Two-tenant VPS placement** — ChadAI and Ask Ebbi on `alwyz-01-dock` as isolated Docker projects | Tony / ChadAI Codex / Ask Ebbi Codex | **done** | ChadAI is already running in `/home/anwhelan/apps/chadai-prod`; Ask Ebbi gets a separate Compose project, networks, volumes, Hermes home and credentials. Ask Ebbi deployment is still pending. |
| M20 | **Kanban Surface v2 — cross-project AgentOS execution layer** — collab-mem project cards define isolated native Hermes boards; one UI/MCP surface reads all projects while mutations stay scoped. Card: `projects/kanban-surface.md` | Tony / Codex | **active** | Local implementation is green against Hermes Agent v0.19.0. Next = choose the dedicated KSM host, bind verified 1Password references and KBA runtime policies, then run the five live smoke paths before deployment. |

> **2026-07-17 note:** M19 is resolved: the new operating boundary is one VPS with two isolated Docker projects. ChadAI work is owned by the separate ChadAI Codex lane; Ask Ebbi work is owned by this lane. The two stacks must not share volumes, networks, Hermes state or secrets.
