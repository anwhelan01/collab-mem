...
| M18 | **Ask Ebbi — the Planning Oracle** — Hermes-native (Ebbi + 4 sub-profiles), Library MCP, 91k appeals. Repo consolidated 2026-07-12 to `k3ss-official/ask-ebbi-3` (was ask-ebbi-2). Card: `projects/ask-ebbi-3.md` | Tony / Rae | **active** | Live at askebbi.com via Cloudflare tunnel → Docker `ebbi` :8051. Next = land the responsiveness fix (overflow + iPad portrait) on `ui-console-alt`. |
