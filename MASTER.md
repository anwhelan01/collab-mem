...
| M18 | **Ask Ebbi 3 — the Planning Oracle** — Hermes-native Ask Ebbi product, Library MCP and active private UAT. Card: `projects/ask-ebbi-3.md` | Tony + Rae | **active UAT** | Live on `alwyzon-1` as isolated Docker project `ask-ebbi`, container `ebbi`, deployed revision `bd16dc04` from `feat/uncle-demo-feedback`; `askebbi.com` is protected by Cloudflare Access. Continue Tony/Rae UAT, then decide promotion to `main`. |
